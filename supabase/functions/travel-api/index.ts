...
import { flightFeed, flightPosition, historyEnabled } from './flights.ts';
...
async function dispatch(actor: string, method: string, path: string, body: unknown = {}) {
...
const paths = (d: any) =>
  (d?.places ?? []).flatMap((p: any) => p.photos ?? []).map((p: any) => p.storagePath).filter(
    Boolean,
  );
async function removePhotos(keys: string[]) {
  if (keys.length) {
    await platform('/storage/v1/object/journey-photos', 'DELETE', { prefixes: keys });
  }
}
  ...
  for (const p of result.places) {
    for (const photo of p.photos) {
      const raw = decodePhoto(photo.jpeg),
        path = `${uid}/${d.id.toLowerCase()}/${version}/${crypto.randomUUID()}.jpg`;
...
export async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const network = await digest(
      (req.headers.get('x-forwarded-for') || 'unknown').split(',').pop()!.trim(),
    );
    await limit('request:' + network, 240);
    if (path.startsWith('/s/') && method === 'GET') {
      const value = path.slice(3);
      requireValue(/^[a-f0-9]{80}$/.test(value), 'This link is unavailable.', 404);
      await limit('share:' + network, 30);
      const links = await platform(
        '/rest/v1/travel_links?select=document_id&token_hash=eq.' + await digest(value),
      );
      requireValue(links[0], 'This link was revoked or is no longer available.', 404);
      ...
      const remote = await hydrate(await rpc('travel_remote', { d: docs[0], actor: null }));
      if (q.get('format') === 'json') return json(remote);
      if (q.get('format') === 'txt') {
        return new Response(
          sharedLines(remote.document, remote.owner.name).map((l) =>
            l.photo ? '[Photo included in the PDF and JSON]' : l.text + (l.url ? ' ' + l.url : '')
          ).join('\n\n'),
    ...
    if (['/v1/auth/register', '/v1/auth/login'].includes(path) && method === 'POST') {
      await limit('auth:' + network, 20, 300);
      await limit('auth-handle:' + await digest(String(body.handle ?? '').toLowerCase()), 15, 300);
      if (path.endsWith('/register')) await limit('registration-global', 50, 3600);
    ...
    if (path === '/v1/account' && method === 'DELETE') {
      const docs = await platform('/rest/v1/travel_documents?select=body&owner_id=eq.' + uid);
    ...
    if (
      method === 'GET' &&
      ['/v1/flights/status', '/v1/flights/history', '/v1/flights/position'].includes(path)
    ) {
      ...
      return json(
        path.endsWith('/position')
          ...
          : await flightFeed(q.get('q') || '', q.get('date') || '', path.endsWith('/history')),
      );
    }
    ...
    if (path === '/v1/places/search' && method === 'GET') {
      ...
      return json(await searchPlaces(q.get('q') || '', q.get('category') || 'attractions'));
    }
    if (/^\/v1\/places\/\d+$/.test(path) && method === 'GET') {
      await limit('places:' + uid, 30);
      return json(await placeDetails(path.split('/').pop()!));
    }
    if (['/v1/ai/activities', '/v1/ai/hotel'].includes(path) && method === 'POST') {
      await limit('ai:' + uid, 10, 3600);
      return json(
        path.endsWith('hotel') ? await overview(body) : await recommend(body.city, body.interests),
      );
    }
    const match = path.match(/^\/v1\/documents\/([a-fA-F0-9-]{36})(?:\/(link|revoke))?$/);
    if (match) {
    ...
    if (e instanceof SyntaxError) return json({ error: 'Malformed JSON request.' }, 400);
    return json(
      { error: 'The cloud service could not complete this request. Please try again.' },
      500,
    );
  }
}
