...
export function requireValue(ok: unknown, message: string, status = 400): asserts ok {
  if (!ok) throw new Problem(message, status);
}
export const object = (v: any) => v !== null && typeof v === 'object' && !Array.isArray(v);
...
export function safeURL(v: any) {
  try {
    const u = new URL(v);
    return typeof v === 'string' && v.length <= 5000 && ['http:', 'https:'].includes(u.protocol) &&
...
export function place(p: any, empty = false) {
  ...
  for (const k of ['city', 'address', 'phone', 'website', 'source', 'overview']) {
  ...
  if (p.rating != null) {
    requireValue(finite(p.rating) && p.rating >= 0 && p.rating <= 5, 'Invalid provider rating.');
  }
}
export function money(v: any) {
  if (v == null) return;
  requireValue(object(v) && finite(v.amount) && v.amount >= 0 && v.amount <= 1e9, 'Invalid price.');
  requireValue(
...
export function validateDocument(d: any) {
  ...
  requireValue(['journey', 'trip', 'itinerary'].includes(d.kind), 'Invalid journey type.');
  requireValue(['private', 'friends', 'public'].includes(d.visibility), 'Invalid audience.');
  ...
  for (
    ...
    requireValue(Array.isArray(d[k]) && d[k].length <= limit, `Too many or invalid ${k}.`);
    requireValue(d[k].every((v: any) => object(v) && uuid(v.id)), `Invalid ${k} identifiers.`);
    requireValue(
  ...
  for (const s of d.stops) {
    ...
    requireValue(
      Number.isInteger(s.nights) && s.nights >= 1 && s.nights <= 365 && day(s.arrival),
    ...
    if (s.latitude != null || s.longitude != null) {
      requireValue(
        finite(s.latitude) && finite(s.longitude) && Math.abs(s.latitude) <= 90 &&
  ...
  for (const e of d.events) {
    ...
    requireValue(e.allDay == null || typeof e.allDay === 'boolean', 'Invalid all-day setting.');
    requireValue(
  ...
  for (const h of d.hotels) {
    ...
    requireValue(
      Number.isInteger(h.guests) && h.guests >= 1 && h.guests <= 99 && Number.isInteger(h.rooms) &&
  ...
  for (const f of d.flights) {
    for (const k of ['airline', 'departureAirport', 'arrivalAirport']) text(f[k], k, 300);
    for (const k of ['departureDay', 'arrivalDay']) requireValue(day(f[k]), 'Invalid flight date.');
    ...
    requireValue(!f.bookingLink || safeURL(f.bookingLink), 'Invalid booking link.');
    money(f.cost);
  }
  let total = 0;
  for (const p of d.places) {
    place(p.place);
    requireValue(finite(p.overall) && p.overall >= 0 && p.overall <= 10, 'Invalid personal score.');
    requireValue(
      object(p.scores) && Object.keys(p.scores).length <= 20 &&
        Object.entries(p.scores).every(([k, v]) =>
          k.length <= 100 && finite(v) && (v as number) >= 0 && (v as number) <= 10
        ),
      'Invalid category scores.',
    );
    requireValue(p.visitedOn == null || day(p.visitedOn), 'Invalid visit date.');
    requireValue(
      p.michelinStars == null ||
        (Number.isInteger(p.michelinStars) && p.michelinStars >= 0 && p.michelinStars <= 3),
    ...
    requireValue(
      new Set(p.photos.map((v: any) => v.id?.toLowerCase())).size === p.photos.length,
  ...
  requireValue(total <= 28000000, 'This shared journey exceeds the 28 MB photo limit.');
}
