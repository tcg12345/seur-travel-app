...
import { day, Problem, safeURL, validateDocument } from '../functions/travel-api/validation.ts';
...
Deno.test('Departure dates follow origin time zone across midnight', () => {
  assert.equal(
    localDay({ scheduled_out: '2026-09-08T01:00:00Z', origin: { timezone: 'America/New_York' } }),
    '2026-09-07',
  );
  assert.equal(
    localDay({ scheduled_out: '2026-09-08T01:00:00Z', origin: { timezone: 'Asia/Tokyo' } }),
    '2026-09-08',
  );
});
