...
The flight detail sheet uses 12-point outer margins with route, airport times and punctuality immersed directly in the page. A flat status ribbon introduces the departure countdown. Compact adjacent gate/terminal tiles and separate aircraft/registration tiles organize supporting details without nesting cards.

The always-visible detailed timetable follows gate departure, taxi to runway, takeoff, landing, taxi to gate and gate arrival. Scheduled, estimated and actual values remain separate and show airport-local dates. Taxi durations require a valid pair of timestamps; unavailable values show a dash. Saved flights without live data retain their booked gate schedule. Runway estimates pass through the existing provider response without additional requests. Tracking, alerts, history and booking notes remain below the timetable; accessibility text sizes stack the time columns.

Punctuality is evaluated independently for departure and arrival. Actual timestamps take precedence over estimates, then reported delay seconds provide a fallback. Green means on time, blue early, amber late/diverted, red cancelled, and neutral means timing is not confirmed. Text and symbols repeat the meaning so color is never the only cue. Before departure the status accent accounts for either endpoint’s delay; after departure it follows arrival timing. Live departure countdowns use the expected gate time; saved-only countdowns remain explicitly labeled as scheduled.
