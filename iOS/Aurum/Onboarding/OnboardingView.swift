...
struct OnboardingView: View {
    ...
    var body: some View {
        Group {
            if step == 0 { welcome.preferredColorScheme(.dark) }
                ...
                MembershipPreviewView(onBack: { move(to: 3) }, onFinish: finish)
            } else { preferences }
        }
        ...
        GeometryReader { geo in
            ZStack {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    Button("Explore first") { move(to: 4) }.font(.subheadline).dynamicTypeSize(...DynamicTypeSize.accessibility1).foregroundStyle(.white).frame(maxWidth: .infinity, minHeight: 44).accessibilityIdentifier("onboarding-explore")
    ...
    private var preferences: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 7) {
                ForEach(1...4, id: \.self) { value in Capsule().fill(value <= step ? Color.bronze : Color.bronze.opacity(0.15)).frame(height: 3) }
            }.padding(.horizontal, 27).padding(.top, 20).padding(.bottom, 6).accessibilityElement(children: .ignore).accessibilityLabel("Step \(step) of 4")
            ScrollView {
            ...
            VStack(spacing: 9) {
                Button { if review && step == 3 { dismiss() } else { move(to: step + 1) } } label: {
                    HStack { Text(step == 3 ? (review ? "Save preferences" : "See your membership") : "Continue"); Spacer(); Image(systemName: "arrow.right") }.onboardingPrimary()
