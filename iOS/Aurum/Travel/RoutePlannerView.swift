        ...
        NavigationStack {
            List {
                ...
                .sheet(item: $locating) { stop in RouteLocationPicker(title: "Locate " + stop.name, initial: stop.name) { point in
                    if let i = stops.firstIndex(where: { $0.id == stop.id }) { stops[i].name = point.name; stops[i].latitude = point.latitude; stops[i].longitude = point.longitude }
                } }
        ...
        NavigationStack {
            Form {
                LocationAutocompleteField("Search city", text: $text, kind: .city, identifier: "route-city-query", onEdit: { point = nil }) { location in
                    if let lat = location.place.latitude, let lon = location.place.longitude { point = RoutePoint(id: "", name: location.text, latitude: lat, longitude: lon) }
                }
