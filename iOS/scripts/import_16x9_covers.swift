...
// Usage: swift import_16x9_covers.swift <source-folder> <asset-catalog> <selection-json>
...
precondition(args.count == 4, "Supply source, asset catalog, and explicit selection JSON")
...
let entries = try JSONDecoder().decode([Source].self, from: Data(contentsOf: source.appendingPathComponent("index.json")))
...
for entry in entries where selected.contains(entry.destination) {
    try autoreleasepool {
        ...
        precondition(w * 9 == h * 16 && w >= 1280, "Expected 16:9 image: \(entry.destination)")
        ...
        guard let image = CGImageSourceCreateThumbnailAtIndex(decoded, 0, options as CFDictionary) else { fatalError("Cannot resize") }
        precondition(image.width == 1280 && image.height == 720)
        ...
        let photo: [String: Any] = ["provider": "bundled", "imageURL": entry.original_url, "sourceURL": entry.source_page,
            "authors": [["name": entry.photographer]], "license": entry.license, "licenseURL": entry.license_url,
            "title": entry.photo_title, "attribution": entry.attribution.isEmpty ? "Photo by " + entry.photographer + " · " + entry.license : entry.attribution]
